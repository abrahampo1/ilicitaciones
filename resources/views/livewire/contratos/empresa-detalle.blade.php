<div>
    @section('meta_title', $this->empresa->nombre . ' - Adjudicaciones - I-Licitaciones')
    @section('meta_description', 'Adjudicaciones y contratos de ' . $this->empresa->nombre . '. Historial de licitaciones ganadas e importes totales.')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="relative mb-12">
            <div class="absolute inset-0 bg-gradient-to-r from-sky-500/10 via-cyan-500/5 to-transparent rounded-3xl blur-3xl pointer-events-none"></div>
            <div class="relative">
                <h1 class="text-3xl md:text-4xl font-light leading-tight mb-3 text-neutral-100">
                    {{ $this->empresa->nombre }}
                </h1>
                <div class="flex flex-wrap items-center gap-4 text-sm">
                    @if($this->empresa->identificador)
                        <span class="font-mono text-neutral-400 bg-neutral-900/50 px-3 py-1 rounded-full border border-neutral-800">
                            ID: {{ $this->empresa->identificador }}
                        </span>
                    @endif
                    @if($this->empresa->nif && $this->empresa->nif !== $this->empresa->identificador)
                        <span class="font-mono text-neutral-400 bg-neutral-900/50 px-3 py-1 rounded-full border border-neutral-800">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                @if($adjudicaciones->count() > 0)
                    <div class="space-y-4">
                        @foreach ($adjudicaciones as $adj)
                            <a href="{{ route('contratos.show', $adj->licitacion_id) }}" wire:navigate
                                ...
                                <div class="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-3">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @if($adj->licitacion && $adj->licitacion->organismo)
                                            <p class="text-xs text-cyan-400/70 mt-2 truncate">
                                ...
                                <div class="flex flex-wrap items-center gap-y-2 gap-x-4 text-xs">
                                    ...
                                    @if($adj->tipo_procedimiento)
                                        <span class="px-2.5 py-0.5 rounded-full border border-neutral-700/50 bg-neutral-800/50 text-neutral-400">
                                            {{ \Modules\Contratos\Models\Licitacion::PROCEDIMIENTO_LABELS[$adj->tipo_procedimiento] ?? $adj->tipo_procedimiento }}
                                        </span>
                                    @endif
                                    @if($adj->urgencia)
                                        <span class="px-2.5 py-0.5 rounded-full border border-amber-500/20 bg-amber-500/10 text-amber-400">
                                            {{ $adj->urgencia }}
                                        </span>
                                    @endif
                                </div>
            ...
            <div class="space-y-6">
                ...
                <!-- Related Companies -->
                <div class="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-6">
                    <h3 class="flex items-center gap-2 text-neutral-300 text-sm font-medium mb-2">Empresas Relacionadas</h3>
                    <p class="text-xs text-neutral-400 mb-6">Tienen el mismo identificador pero distinto nombre.</p>
                    ...
                    @foreach($relatedCompanies as $relatedCompany)
                        <div class="flex items-center gap-2 p-1 px-2 mt-2 border border-neutral-800 rounded-lg hover:bg-neutral-800 transition-colors">
                            <a href="{{ route('empresas.show', $relatedCompany->id) }}" wire:navigate class="flex items-center gap-2">
                                <div>
                    ...
                    @if ($relatedCompanies->isEmpty())
                        <p class="text-center italic text-xs text-neutral-400">No hay empresas relacionadas</p>
                    @endif
                </div>
            </div>
        </div>
